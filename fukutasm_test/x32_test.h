...
void x32_test_internl(fukutasm::fuku_assambler_ctx& ctx) {
...
#define op FUKU_REG_EAX
    ...
    uint64_t vall = 0x1000000;
        ...
        asm_def_1op_test(aam, , imm, FUKU_OPERAND_SIZE_32)
        asm_def_1op_test(aad, , imm, FUKU_OPERAND_SIZE_32)
        ...
        //asm_def_1op_test(nop, , 1, FUKU_OPERAND_SIZE_32);
...
void x32_test() {
    ...
    fuku_assambler fasm(FUKU_ASSAMBLER_ARCH_X86);

    fuku_inst inst;

    fukutasm::fuku_assambler_ctx& ctx = fasm.get_context();
    ctx.arch = FUKU_ASSAMBLER_ARCH_X86;
    ctx.short_cfg = 0xFF;
    ctx.inst = &inst;
    ...
    for (size_t i_1 = 0; i_1 < 2; i_1++) {
        for (size_t i_2 = 0; i_2 < 2; i_2++) {
            for (size_t i_3 = 0; i_3 < 2; i_3++) {
                ...
                printf("START CHECK [X32] [EAX_SHORT : %s] [DISP_SHORT : %s] [IMM_SHORT : %s] \n", 
                    (i_1 ? "ON " : "OFF"),
                    (i_2 ? "ON " : "OFF"),
                    (i_3 ? "ON " : "OFF")
                );

                x32_test_internl(ctx);
            }
        }
    }
