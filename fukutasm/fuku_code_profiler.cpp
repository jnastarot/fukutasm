...
uint64_t fuku_code_profiler::profile_graph_registers(fuku_code_holder& code, inststorage::iterator lines_iter) {
    uint64_t included_registers = 0;
    uint64_t excluded_registers = 0;

    cs_insn* instruction;

    fuku_reg_access op_access[32];
    ...
    for (; lines_iter != code.get_insts().end(); ++lines_iter) {
        auto& current_inst = *lines_iter;

        cs_disasm(cap_handle, current_inst.get_opcode(), current_inst.get_oplength(), 0, 1, &instruction);
        ...
        for (size_t access_idx = 0; access_idx < reg_idx; access_idx++) {
            ...
            if (op_access[access_idx].access & REGISTER_ACCESS_READ) {
                current_excluded_registers |= get_flag_complex_register(op_access[access_idx].reg);
            }
            if (op_access[access_idx].access & REGISTER_ACCESS_WRITE) {
                current_included_registers |= get_flag_complex_register_by_size(op_access[access_idx].reg);
            }
        }
...
uint64_t fuku_code_profiler::profile_graph_eflags(fuku_code_holder& code, inststorage::iterator lines_iter) {
    ...
    for (; lines_iter != code.get_insts().end(); ++lines_iter) {
        ...
        uint16_t current_id = current_inst.get_id();
        uint64_t current_eflags = current_inst.get_cpu_flags();
...
bool fuku_code_profiler::profile_code(fuku_code_holder& code) {
    ...
    for (auto line_iter = code.get_insts().begin(); line_iter != code.get_insts().end(); ++line_iter) {
        ...
        (*line_iter).set_cpu_flags(profile_graph_eflags(code, line_iter));
        (*line_iter).set_cpu_registers(profile_graph_registers(code, line_iter));
    }
    ...
    for (auto line_iter = code.get_insts().begin(); line_iter != code.get_insts().end(); ++line_iter) {
        ...
        case X86_INS_JMP: {
            ...
            if (line_iter->get_rip_reloc() && !line_iter->is_used_disp_reloc()) {

                if (line_iter->get_rip_reloc()->label->has_linked_instruction) {
                    line_iter->set_cpu_registers(line_iter->get_rip_reloc()->label->inst->get_cpu_registers());
                }
            }
            break;
        }
        ...
        case X86_INS_JP:  case X86_INS_JS: {
            ...
            if (line_iter->get_rip_reloc() && !line_iter->is_used_disp_reloc()) {
                ...
                if (line_iter->get_rip_reloc()->label->has_linked_instruction) {
                    ...
                    if (next_line != code.get_insts().end()) {
                        line_iter->set_cpu_registers(line_iter->get_rip_reloc()->label->inst->get_cpu_registers() & next_line->get_cpu_registers());
                    }
                }
            }
            break;
...
inline void push_access(uint8_t& reg_idx, fuku_reg_access&& access, fuku_reg_access op_access[]) {
    op_access[reg_idx] = access;
    reg_idx++;
}

void get_operand_access(uint8_t& reg_idx, cs_insn* instruction, uint8_t op_num, fuku_reg_access op_access[], uint64_t table[], uint8_t default_access) {
...
bool fuku_code_profiler::get_instruction_operands_access(cs_insn* instruction, uint8_t& reg_idx, void* _op_access) {

    reg_idx = 0;

    fuku_reg_access* op_access = (fuku_reg_access*)_op_access;
