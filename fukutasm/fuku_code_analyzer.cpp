...
bool fuku_code_analyzer::analyze_code(
    ...
    if (count) {
        ...
        for (size_t insn_idx = 0; insn_idx < count; insn_idx++) {
            fuku_inst &line = analyzed_code.add_inst();

            auto& current_insn = insn[insn_idx];
           
            line.set_source_address(virtual_address + current_insn.address)
                .set_current_address(virtual_address + current_insn.address)
                .set_opcode(&src[current_insn.address], (uint8_t)current_insn.size)
                .set_cpu_flags(current_insn.detail->x86.eflags)
                .set_id(current_insn.id)
                .set_inst_flags(current_insn.detail->x86.encoding.disp_offset << 8 | current_insn.detail->x86.encoding.imm_offset);
            ...
            for (uint8_t op_idx = 0; op_idx < current_insn.detail->x86.op_count;op_idx++) {
                ...
                if (operand.type == X86_OP_MEM) {
                    ...
                    if (operand.mem.base == X86_REG_RIP) {
                        ...
                        line.set_rip_reloc(
                            analyzed_code.create_rip_relocation(
                                fuku_rip_relocation()
                                .set_offset(current_insn.detail->x86.encoding.disp_offset)
                                .set_label(analyzed_code.create_label(
                                    fuku_code_label().set_address(virtual_address + X86_REL_ADDR(current_insn))
                                )
                                )
                            )
                        );
                ...
                case  X86_INS_LOOP: case X86_INS_LOOPE: case X86_INS_LOOPNE: {
                    ...
                    if (current_insn.detail->x86.operands[0].type == X86_OP_IMM) {
                        ...
                        line.set_rip_reloc(
                            analyzed_code.create_rip_relocation(
                                fuku_rip_relocation()
                                .set_offset(current_insn.detail->x86.encoding.imm_offset)
                                .set_label(analyzed_code.create_label(
                                    fuku_code_label().set_address(virtual_address + (int32_t)(current_insn.detail->x86.operands[0].imm))
                                )
                                )
                            )
                        );
        ...
        if (relocations) {
            ...
            for (auto reloc : *relocations) { //associate relocs
                ...
                fuku_inst * line = analyzed_code.get_source_inst_range(reloc.virtual_address);

                if (line) {
                    uint8_t  reloc_offset = (uint8_t)(reloc.virtual_address - line->get_current_address());
                    uint64_t reloc_dst = ((code.get_arch() == FUKU_ASSAMBLER_ARCH_X86) ?
                        *(uint32_t*)&line->get_opcode()[reloc_offset] :
                        *(uint64_t*)&line->get_opcode()[reloc_offset]);


                    if (reloc_offset == (line->get_inst_flags() & 0xFF) ) {

                        line->set_imm_reloc(
                            analyzed_code.create_relocation(
                                        fuku_relocation()
                                            .set_offset(reloc_offset)
                                            .set_label(
                                                    analyzed_code.create_label(
                                                        fuku_code_label().set_address(reloc_dst))
                                                    )
                                            .set_id(reloc.relocation_id)
                                        )                   
                            );

                    } else if (reloc_offset == ((line->get_inst_flags() >> 8) & 0xFF) ) {
